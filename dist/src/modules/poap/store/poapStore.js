...
const initialWhitelistedWallets = [
...
export const usePoapStore = create()(persist((set, get) => ({
    // Initial state
    ...
    claimPOAP: async (wallet) => {
        const { checkIfWalletAlreadyClaimed, getWalletAssignedLink, mintLinks, setClaimStatus } = get();
        set({ isLoading: true });
        ...
        set(state => ({
            mintLinks: state.mintLinks.map(link => link.id === selectedLink.id
                ? {
                    ...link,
                    claimed: true,
                    claimedBy: wallet,
